class Email {
    ...
    sendEmail(): void {
        console.log(`Email sent from ${this.from} to ${this.to} - Subject: ${this.subject}`);
...
class EmailPool {
    ...
    private constructor() {
        for (let i = 0; i < 5; i++) {
            this.availableEmails.push(new Email('example@example.com', 'recipient@example.com', 'SubjectSample', 'MessageSample'));
        }
    }
